 ...
 */
export function encodeMouseCommandBinary(commandType, x, y, button = 0, sx = 0, sy = 0) {
 ...
 */
export function decodeMouseCommandBinary(buffer) {
...
/**
 * Normalize payload - decode binary if needed, pass through JSON
 * @param {ArrayBuffer|Object} payload - Binary or JSON payload
 * @param {string} commandType - Expected command type (for logging/validation)
 * @returns {Object} Normalized payload
 */
export function normalizePayload(payload, commandType) {
  // If payload is binary (ArrayBuffer), decode it
  if (payload instanceof ArrayBuffer) {
    const decoded = decodeMouseCommandBinary(payload);
    if (decoded) {
      return decoded;
    }
  }
